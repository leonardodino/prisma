...
use connector::{
    ...
    ConnectorResult, RelationCompare, ScalarCompare,
};
use prisma_models::*;
...
pub trait Transaction {
    ...
    fn filter_ids_by_parents(
        ...
        let base_filter = parent_field.every_related(related_model.fields().id().is_in(parent_ids));

        let filter = selector
            .into_iter()
            .fold(base_filter, |acc, filter| Filter::and(vec![acc, filter]));
